...
for sector in data.keys():
    ...
    for date in data[sector].keys():
        if date not in result[sector]:
            result[sector][date] = {}
        for channel in data[sector][date].keys():
            # 채널이 아직 결과에 없으면 추가
            if channel not in result[sector][date]:
                result[sector][date][channel] = {
                    "posts": []
                }
            
            # 채널 데이터가 리스트인 경우, 각 포스트의 'content' 필드만 추출
            for post in data[sector][date][channel]['posts']:
                result[sector][date][channel]["posts"].append(post['content'])
        print(sector,date)
        print(result[sector][date])
        print('--------------------------------')
